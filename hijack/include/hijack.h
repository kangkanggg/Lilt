...
#define LOGGER(level, format, ...)                              \
  ({                                                            \
    char *_print_level_str = lilt_getenv("LOGGER_LEVEL");            \
    int _print_level = 3;                                       \
    if (_print_level_str) {                                     \
...
/**
 * Resolve a symbol from the real NVIDIA CUDA driver loaded by the proxy.
 * The returned address never comes from the proxy itself.
 */
void *cuda_real_symbol(const char *symbol);

/** Call the glibc dlsym implementation without re-entering the TGS hook. */
void *lilt_system_dlsym(void *handle, const char *symbol);

/**
 * Account one kernel launch using the policy selected by the HP/BE proxy.
 */
void lilt_before_kernel_launch(uint64_t blocks);

/** Account a launch while exposing metadata used by adaptive BE policies. */
void lilt_before_kernel_launch_ex(CUfunction function, uint64_t blocks,
                                 CUstream stream,
                                 int per_thread_default_stream);

/** Complete policy bookkeeping after the real launch returns. */
void lilt_after_kernel_launch(CUstream stream, int per_thread_default_stream,
                             CUresult launch_result);

/** Force all tracked BE work to complete at a Graphlet boundary. */
